...
def get_device_applist():
    # 和设备的lockdown service连接上
    get_device_list()
    ...
    print(f'----->请求访问指定服务：{SERVICE_NAME}')
    sock.sendall(data)
    b = sock.recv(4)
    payload = loads_plist(sock, b, is_fist=False)
    print(f'----->接收访问服务的回包：{payload}')
    print(f'----->请求访问指定地址: {payload["Port"]}')
    if payload.get('EnableSessionSSL'):
    ...
    while 1:
        b = sock.recv(4)
        payload = loads_plist(sock, b, is_fist=False)
        # print(f'----->app list 请求回包数据：{payload}')
        if payload['Status'] == 'Complete':
            break
        for i in payload['CurrentList']:
            print(i['CFBundleIdentifier'])
